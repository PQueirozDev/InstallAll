...
export function Downloader() {
  ...
  const [activeDownload, setActiveDownload] = useState<{ info: MediaInfo; format: MediaFormat } | null>(null);
  ...
  useEffect(() => { if (job?.status !== "ready" || !activeDownload) return; const item = { title: activeDownload.info.title, platform: platformName[activeDownload.info.platform], format: activeDownload.format.label, date: Date.now() }; const next = [item, ...history].slice(0, 8); setHistory(next); try { localStorage.setItem("installall-history", JSON.stringify(next)); } catch {} window.location.assign(`${API_URL}${job.downloadUrl}`); /* only once */ // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [job?.status]);
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); setLoading(true); setError(""); setInfo(null); setJob(null); const submittedUrl = url.trim(); try { const value = await analyze(submittedUrl); setAnalyzedUrl(submittedUrl); setInfo(value); const first = value.formats.find((f) => f.kind === "video") ?? value.formats[0]; setKind(first?.kind ?? "video"); setFormatId(first?.id ?? ""); } catch (e) { setError(e instanceof Error ? e.message : "Link inválido."); } finally { setLoading(false); } }
  async function download() { if (!selected || !info || selected.kind !== kind || starting) return; setActiveDownload({ info, format: selected }); setStarting(true); setError(""); try { setJob(await startDownload(analyzedUrl, selected.id)); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível iniciar o download."); } finally { setStarting(false); } }
  ...
  return <main>
    ...
    {info && <section className="result shell" aria-live="polite"><div className="preview">{info.thumbnail ? <img src={info.thumbnail} alt="Thumbnail do vídeo"/> : <div className="noPreview">Sem preview</div>}<span>{duration(info.duration)}</span></div><div className="details"><div className="platformTag">{platformName[info.platform]}</div><h2>{info.title}</h2>{info.author && <p className="author">por {info.author}</p>}
      ...
      {starting && <p role="status">Preparando download...</p>}
      {(job?.status === "ready" || job?.status === "failed") && <button onClick={() => setJob(null)}>Escolher outro formato / tentar novamente</button>}
