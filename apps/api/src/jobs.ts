...
export const sanitizeFilename = (name: string) => name.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9._ -]/g, "").trim().replace(/\s+/g, "-").slice(0, 100) || "video";
...
export function getJob(id: string, owner: string): DownloadJob { const job = jobs.get(id); if (!job || job.owner !== owner) throw new AppError(404, "JOB_NOT_FOUND", "Download não encontrado."); return expose(job); }
export async function getJobFile(id: string, owner: string) { const job = jobs.get(id); if (!job || job.owner !== owner || job.status !== "ready" || !job.filePath) throw new AppError(404, "FILE_NOT_FOUND", "Arquivo não disponível."); return { path: job.filePath, filename: job.filename!, size: (await stat(job.filePath)).size }; }
